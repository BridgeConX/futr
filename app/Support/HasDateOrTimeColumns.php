...
trait HasDateOrTimeColumns {
    ...
    {
        $this->attributes[$key] = $value ? Carbon::createFromFormat($this->getDateUserFormat(), $value) : null;
    }
    ...
    {
        $this->attributes[$key] = $value ? Carbon::createFromFormat($this->getTimeUserFormat(), $value) : null;
    }
    ...
    protected function getDateColumnValue($key)
    {
        ...
        if($value instanceof Carbon)
            $value = $value->format($this->getDateSourceFormat());

        if( ! $value )
            return false;

        $carbon = Carbon::createFromFormat($this->getDateSourceFormat(), $value)->startOfDay();
    ...
    protected function getTimeColumnValue($key)
    {
        ...
        if($value instanceof Carbon)
            $value = $value->format($this->getTimeSourceFormat());
